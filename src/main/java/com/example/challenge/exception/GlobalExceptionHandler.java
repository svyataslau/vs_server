...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<AppError> handleInvalidFieldException(InvalidFieldException exception) {
        return new ResponseEntity<>(
                new AppError(HttpStatus.BAD_REQUEST.value(), exception.getMessage()), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler(UnauthorizedException.class)
    public ResponseEntity<AppError> handleUnauthorizedException(UnauthorizedException exception) {
        return new ResponseEntity<>(
                new AppError(HttpStatus.UNAUTHORIZED.value(), exception.getMessage()), HttpStatus.UNAUTHORIZED);
    }

    @ExceptionHandler(CustomServerException.class)
    public ResponseEntity<AppError> handleException() {
        return new ResponseEntity<>(
                new AppError(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Something has gone wrong..."),
